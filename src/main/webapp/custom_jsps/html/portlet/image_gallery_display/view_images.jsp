			...
			<c:when test="<%= fileEntry != null %>">
				...
				<c:if test="<%= DLFileEntryPermission.contains(permissionChecker, fileEntry, ActionKeys.VIEW) %>">
					...
					if (PropsValues.DL_FILE_ENTRY_PREVIEW_ENABLED) {
						if (hasAudio) {
							...
							href = DLUtil.getPreviewURL(fileEntry, fileVersion, themeDisplay, "&supportedVideo=1&mediaGallery=1");
                                                        src = themeDisplay.getPathThemeImages() + "/file_system/large/video.png";

							for (String videoContainer : PropsValues.DL_FILE_ENTRY_PREVIEW_VIDEO_CONTAINERS) {
                                                                dataOptions += "&" + videoContainer + "PreviewURL=" + HtmlUtil.escapeURL(mimeType);
							}
						}
					}
					%>
					...
					<div class="image-icon">
						<a class="image-link preview" <%= (hasAudio || hasVideo) ? "data-options=\"height=" + playerHeight + "&thumbnailURL=" + fileEntry.getFileEntryId() + "&width=640" + dataOptions + "\"" : StringPool.BLANK %> href="<%= href %>" thumbnailId="<%= thumbnailId %>" title="<%= HtmlUtil.escape(fileEntry.getTitle()) + " - " + HtmlUtil.escape(fileEntry.getDescription()) %>">
